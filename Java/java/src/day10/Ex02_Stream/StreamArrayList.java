...
public class StreamArrayList {
	...
	public static void main(String[] args) {
		...
		// filter → 필터
		// ㄴ 스트림 요소에서 조건에 맞는 요소만 선택하는 연산
		// 가격이 200원 이상인 상품만 필터링하여 반복

		productList.stream().filter(p -> p.getPrice() >= 2000).forEach(System.out::println);
		System.out.println();

		// 상품명이 3글자인 상품만 필터링하여 반복

		productList.stream().filter(p -> p.getName().length() == 3).forEach(System.out::println);
		System.out.println();

		// sorted - 정렬
		// 스트림 요소에서 지정한 기준에 따라 요소들을 오름차순/내림차순으로 정렬하는 연산

		// 이름순 - 오름차순

		System.out.println("이름순으로 정렬 (오름차순)");
		productList.stream().sorted(Comparator.comparing(p -> p.getName())).forEach(p -> System.out.println(p));
		System.out.println();
		// .sorted 안의 문구를 Comparator.comparing(Product::getName) 으로 줄여쓸 수 있다
		...
		// count() - 스트림 요소의 개수
		// 이름이 4글자 이상인 상품의 개수
		long count = productList.stream().filter(p -> p.getName().length() >= 4).count();
		System.out.println("이름이 4글자 이상인 상품들의 수 : " + count);

		// sum - 스트림 요소의 합계
		// 가격이 2000원 미만인 상품들의 가격 합계
		int sum = productList.stream().filter(p -> p.getPrice() < 2000).mapToInt(p -> p.getPrice()).sum();
