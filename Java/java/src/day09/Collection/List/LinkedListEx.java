...
public class LinkedListEx {
	...
	public static void main(String[] args) {
		...
		Product product1 = new Product.ProductBuilder().name("기계식 키보드").price(50000).count(10).build();

		Product product2 = new Product.ProductBuilder().name("무선 마우스").price(30000).count(20).build();

		Product product3 = new Product.ProductBuilder().name("미니 손난로").price(15000).count(11).build();
