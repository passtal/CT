...
public class MultiJoinTest extends Thread {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		s3.start();
		s4.start();
		s1.join();		// 메인 스레드를 s1 스레드 종료까지 일시정지
		s2.join();		// 메인 스레드를 s2 스레드 종료까지 일시정지
		s3.join();		// 메인 스레드를 s3 스레드 종료까지 일시정지
		s4.join();		// 메인 스레드를 s4 스레드 종료까지 일시정지
